    ...
    <div class="indicator-header">
      <div class="filter-block">
        ...
        <div class="filter-block-item">
          <span>分类筛选：</span>
          <span
            :class="[{ selected: classIndex === '0' }, 'filter-item']"
            @click="changeClass('0')"
          >
            全部
          </span>
          <span
            v-for="type in classList"
            :key="type.id"
            :class="[{ selected: classIndex === type.id }, 'filter-item']"
            @click="changeClass(type.id)"
          >
            {{ type.name }}
          </span>
        </div>
    ...
    <a-form :model="form" :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
      ...
      <a-form-item label="类别">
        <a-select v-model:value="form.class_id" placeholder="请选择类别">
          <template
            v-if="typeIndex === '1'"
            v-slot:dropdownRender="{ menuNode: menu }"
          >
            <v-nodes :vnodes="menu" />
            <a-divider style="margin: 4px 0;" />
            <div
              style="padding: 4px 8px; cursor: pointer;line-height: 19px"
              @mousedown="(e) => e.preventDefault()"
              @click="addWorkClass"
            >
              <ehf-icon icon="add-circle-line" style="font-size: 14px" />
              新增类别
            </div>
          </template>
          <a-select-option
      ...
      <a-form-item label="考核标准">
        <a-textarea v-model:value="form.content" :autosize="{ minRows: 10 }" />
        <span class="textarea-tips">
          请输入考核标准，可替换字段用
          <span
            style="color: #1890ff;cursor: pointer"
            @click="handleCopySymbol"
          >
            {num}
          </span>
          代替
  ...
  export default {
    ...
    methods: {
      ...
      handleCopySymbol(e) {
        ...
        const _this = this
        this.$copyText(el.innerText).then(
          function(e) {
